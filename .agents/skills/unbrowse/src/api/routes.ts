...
import { resolveAndExecute } from "../orchestrator/index";
import { getSkill } from "../marketplace/index";
import { executeSkill, rankEndpoints } from "../execution/index";
import { storeCredential } from "../vault/index";
import { interactiveLogin, extractBrowserAuth } from "../auth/index";
import { publishSkill } from "../marketplace/index";
import { recordFeedback, getApiKey } from "../client/index";
import { ROUTE_LIMITS } from "../ratelimit/index";
import type { ProjectionOptions } from "../types/index";
...
export async function registerRoutes(app: FastifyInstance) {
  ...
  app.post("/v1/skills/:skill_id/verify", async (req, reply) => {
    ...
    if (!skill) return reply.code(404).send({ error: "Skill not found" });
    try {
      const { verifySkill } = await import("../verification/index");
